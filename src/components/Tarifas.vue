        ...
        <v-col col=12>
          ...
          >
            <v-container>
              <v-row>
                <v-col lg=6 cols="12">
                  <v-text-field label="Zona" v-model="proceso.zona" :rules="inputRules"></v-text-field>
                </v-col>
                <v-col lg=6 cols="12">
                  <v-text-field label="Tarifa Q" v-model="proceso.tarifa" :rules="inputRules"></v-text-field>
                </v-col>
              </v-row>
            </v-container>
          </v-form>
  ...
  export default {
    ...
    methods: {
      leerDatos(){
        ...
        .then((response) => {
          if(response.status == 200) { 
            this.restartVal();
            this.procesos = response.data.Tarifas;
      ...
      eliminar(){ 
        ...
        api.delete('tarifas', {data: idItem})
        .then((response) => {
          if(response.status == 200) { 
            this.restartVal();
      ...
      Nuevo(){ 
        if(this.$refs.form.validate()){
          const {zona, tarifa} = this.proceso
          const dataProceso = {zona, tarifa}
          api.post('tarifas', dataProceso)
          .then((response) => {
            if(response.status == 200) { 
              this.restartVal();
              this.$refs.SuccessAlert.showMessage(response.data.message);
