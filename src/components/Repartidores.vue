          ...
          >
            <v-container>
              <v-row>
                ...
                <v-col lg=3 cols="12">
                  <v-text-field label="Correo" v-model="proceso.email" :rules="inputRules"></v-text-field>
                </v-col>
                <v-col lg=3 cols="12">
                  <v-text-field label="DPI" v-model="proceso.dpi" :rules="inputRules" type="Number"></v-text-field>
                </v-col>
              </v-row>
            </v-container>
          </v-form>
  ...
  export default {
    ...
    methods: {
      ...
      eliminar(){ 
        ...
        api.delete('repartidores', {data: idItem})
        .then((response) => {
          if(response.status == 200) { 
            this.restartVal();
      ...
      modificar(){ 
        if(this.$refs.form.validate()){
          api.put('repartidores', this.proceso)
          .then((response) => {
            if(response.status == 200) { 
              this.restartVal();
              this.$refs.SuccessAlert.showMessage(response.data.message);
      ...
      Nuevo(){ 
        if(this.$refs.form.validate()){
          const {nombre, cel, email, dpi} = this.proceso
          const dataProceso = {nombre, cel, email, dpi}
          api.post('repartidores', dataProceso)
          .then((response) => {
            if(response.status == 200) { 
              this.restartVal();
              this.$refs.SuccessAlert.showMessage(response.data.message);
